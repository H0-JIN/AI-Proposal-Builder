...
- TXT/MD는 브라우저에서 바로 텍스트를 읽고, PDF/DOCX는 서버 API route에서 메모리 기반으로 텍스트만 추출합니다.
- 업로드 파일 원본은 서버에 저장하지 않으며, 텍스트 추출 처리 후 폐기됩니다.
- 스캔 PDF 또는 이미지 중심 PDF는 1차 버전에서 지원하지 않습니다. 텍스트를 추출할 수 없는 경우 내용을 직접 입력해야 합니다.
- 추출된 텍스트는 길이와 문자 품질 검사를 통과한 경우에만 기존 RFP/브리프 입력창에 반영되며, 사용자가 내용을 확인/수정하거나 추가 메모를 이어 작성한 뒤 AI 분석을 진행할 수 있습니다.
- 추출 텍스트가 100자 미만이거나 깨진 문자/제어문자/바이너리 시그니처가 감지되면 입력창에 자동 삽입하지 않고 안내 메시지를 표시합니다.
