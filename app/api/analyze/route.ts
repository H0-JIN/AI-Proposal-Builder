  ...
  try {
    ...
    const result = await createStructuredJson<AnalysisResult>({
      ...
      system: [
        ...
        'kpiObjectives에는 RFP에 명시된 KPI/성과 목표/평가 지표를 넣고, constraints에는 예산/공간/운영/법규/브랜드 가이드 등 제약을 넣어라. schedule에는 제출 일정, 보고 일정, 오픈 일정, 운영 일정을 분리해 넣어라. 최상위 confirmNeeded에는 실제 과업 범위와 참고 사례 구분이 모호하거나 추가 확인이 필요한 항목만 넣어라.',
