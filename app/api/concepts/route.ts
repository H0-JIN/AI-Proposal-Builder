...
function packetString(source: Record<string, unknown>, key: string) {
  const value = source[key];
  return typeof value === 'string' ? value : '';
}

function packetStringArray(source: Record<string, unknown>, key: string) {
  const value = source[key];
  return Array.isArray(value) ? value.filter((item): item is string => typeof item === 'string') : [];
}

type StrategyInputPacket = {
  projectName: string;
  clientName: string;
  ...
  keyPersuasionTasks: string[];
  brandProductSummary: string;
  productOrServiceTruth: string;
  audiencePerceptionGap: string;
  whatMustBeProven: string[];
  requiredConstraints: string[];
  signatureExperienceOpportunity: string;
  referenceProposalHints: string[];
  forbiddenPatterns: string[];
};

function buildStrategyInputPacket(body: {
  input: ProjectInput;
  analysis: AnalysisResult;
  rfpDiagnosis: RfpDiagnosis;
  brandProductIntelligence: BrandProductIntelligence;
}): StrategyInputPacket {
  const diagnosis = body.rfpDiagnosis as RfpDiagnosis & Record<string, unknown>;
  const brand = body.brandProductIntelligence as BrandProductIntelligence & Record<string, unknown>;
  return {
    projectName: compactText(body.input.projectName || body.analysis.projectOverview || '', 120),
    clientName: compactText(body.input.clientName || '', 80),
    selectedProposalType: body.input.proposalType ? proposalTypeLabels[body.input.proposalType] : compactText(body.analysis.inferredProposalType || '', 80),
    coreProposalThesis: compactText(diagnosis.coreProposalThesis || diagnosis.coreWinningCondition || diagnosis.strategicIssue || '', 260),
    hiddenNeed: compactText(diagnosis.hiddenNeed || diagnosis.hiddenRequirement || '', 220),
    strategicTension: compactText(diagnosis.strategicTension || diagnosis.strategicIssue || '', 220),
    evaluatorRisk: compactText(packetString(diagnosis, 'evaluatorRisk') || diagnosis.genericProposalFailureReason || packetString(diagnosis, 'riskOrConcern') || '', 220),
    keyPersuasionTasks: compactList([
      diagnosis.persuasionTask,
      ...(diagnosis.requiredPersuasionElements || []),
    ].filter((item): item is string => Boolean(item)), 5, 150),
    brandProductSummary: compactText(packetString(brand, 'summary') || packetString(brand, 'brandEssence') || brand.categoryContext || '', 260),
    productOrServiceTruth: compactText(packetString(brand, 'productOrServiceTruth') || brand.productOrServiceMeaning || packetString(brand, 'coreOffering') || '', 240),
    audiencePerceptionGap: compactText(packetString(brand, 'audiencePerceptionGap') || packetString(diagnosis, 'audiencePerceptionGap') || diagnosis.hiddenNeed || '', 220),
    whatMustBeProven: compactList([
      diagnosis.proofBurden,
      ...(diagnosis.requiredProofElements || []),
      packetString(brand, 'whatMustBeProven'),
    ].filter((item): item is string => Boolean(item)), 6, 150),
    requiredConstraints: compactList([
      ...(body.analysis.constraints || []),
      ...packetStringArray(diagnosis, 'requiredConstraints'),
    ].filter(Boolean), 6, 130),
    signatureExperienceOpportunity: compactText(packetString(brand, 'signatureExperienceOpportunity') || packetString(diagnosis, 'signatureProofIdea') || packetString(diagnosis, 'signatureExperienceOpportunity') || '', 240),
    referenceProposalHints: compactList([
      packetString(brand, 'referenceProposalHints'),
      ...packetStringArray(brand, 'scopedReferenceHints'),
    ].flat().filter((item): item is string => Boolean(item)), 3, 140),
    forbiddenPatterns: compactList([
      'full RFP/proposal/reference text 사용 금지',
      'proposal-type preset direction 금지',
      'previous project direction/old client language 금지',
      'generic fallback card 금지',
      'date/client/event/audience fragment 같은 raw evidence label 금지',
      ...(brand.wordsToAvoid || []),
    ], 10, 120),
  };
}

function hasCurrentRfpEvidence(concept: ConceptCandidate) {
  return Boolean(
    concept.directionSource?.rfpEvidence ||
    concept.rfpGrounding?.length ||
    (concept as ConceptCandidate & { evidenceUsed?: unknown }).evidenceUsed ||
    concept.whyThisDirectionExists ||
    concept.oneLineStrategicBet,
  );
}

function hasBrandProductEvidence(concept: ConceptCandidate) {
  return Boolean(
    (concept.directionSource as typeof concept.directionSource & { brandProductEvidence?: string } | undefined)?.brandProductEvidence ||
    (concept as ConceptCandidate & { evidenceUsed?: unknown }).evidenceUsed ||
    concept.signatureProofIdea?.whyThisProvesTheConcept ||
    concept.conceptLeap?.corePromise ||
    concept.mainStrength,
  );
}

function validateBoundedDirections(concepts: ConceptCandidate[]) {
  const hardRejectReasons: string[] = [];
  const axes = new Set<string>();
  concepts.forEach((concept, index) => {
    const label = (concept.strategicDirectionLabel || '').trim();
    const axis = (concept.directionAxis || concept.strategicDirectionType || '').trim();
    const joined = [label, concept.oneLineStrategicBet, concept.whatThisDirectionEmphasizes, concept.whyThisDirectionExists].join(' ');
    if (!label || !axis) hardRejectReasons.push(`direction_${index + 1}_empty`);
    if (axes.has(axis)) hardRejectReasons.push(`direction_${index + 1}_duplicate_axis`);
    axes.add(axis);
    if (/previous project|prior RFP|WDS|월드\s*디자인|old client/i.test(joined)) hardRejectReasons.push(`direction_${index + 1}_previous_project_contamination`);
    if (isGenericDirectionLabel(label) || BASIC_DIRECTION_PATTERN.test(joined)) hardRejectReasons.push(`direction_${index + 1}_preset_or_generic_label`);
    if (RFP_FACT_DIRECTION_PATTERN.test(label)) hardRejectReasons.push(`direction_${index + 1}_raw_evidence_label`);
    if (!hasCurrentRfpEvidence(concept)) hardRejectReasons.push(`direction_${index + 1}_missing_rfp_evidence`);
    if (!hasBrandProductEvidence(concept)) hardRejectReasons.push(`direction_${index + 1}_missing_brand_product_evidence`);
  });
  if (concepts.length !== DEFAULT_CONCEPT_COUNT) hardRejectReasons.push('direction_count_not_three');
  return hardRejectReasons;
}
...
const FACILITY_LABEL_TOKENS = new Set(['공장', 'factory', 'plant', '시설', 'facility', '견학룸', '견학', '전시관', '홍보관', '체험관', '쇼룸', 'showroom', '매장', '점포', 'store', '공간', 'space', '룸', 'room', '존', 'zone', '관', 'hall', '부스', 'booth', '파빌리온', 'pavilion', '외관', '외부', '내부', '외장', '내장', '파사드', 'facade', '인테리어', '리모델링', '건축', '복도', '입구', '동선', '사인']);

// A user-facing label is generic if it is an axis translation, follows the weak subject+axis template,
// uses a venue/content-format word as its subject, OR is anchored (first/last token) on a facility/space word.
function isGenericDirectionLabel(label: string): boolean {
  ...
  return tokens.some((token) => FACILITY_LABEL_TOKENS.has(token.toLowerCase()) || FACILITY_SUBSTRING.test(token));
}
...
const CONTEXT_NOUN_BLOCKLIST = /^(hero|히어로|콘텐츠|콘텐트|content|영상|video|디자인|design|그래픽|graphic|미디어|media|패널|panel|사이니지|signage|키오스크|kiosk|부스|booth|배너|banner|리플렛|책자|카탈로그|catalog|모형|목업|mockup|기획|운영|operation|제작|구축|설치|시공|견학룸|견학|홍보관|체험관|전시관|쇼룸|showroom|파빌리온|pavilion|전시|exhibition|공간|space|룸|room|존|zone|관|hall|공장|factory|plant|시설|facility|제조시설|생산시설|플랜트|공장동|외관|외부|내부|외장|내장|파사드|facade|인테리어|리모델링|건축|복도|입구|동선|사인|설계)$/i;

// Reject compound facility tokens too (포카리공장, 공장견학, 공장동), which the exact-match blocklist misses.
const FACILITY_SUBSTRING = /(공장|시설|견학|쇼룸|매장|점포|factory|plant|facility|showroom|파빌리온|pavilion|부스|booth|외관|외장|인테리어|리모델링|건축|파사드|facade)/i;
...
const AXIS_SCENE_INTENT: Record<string, (c: string) => string> = {
  ...
  process_trust: (c) => `${c}의 원료·제조·품질 과정을 단계별로 투명하게 보여주는 공정 신뢰 타임라인`,
  signature_scene: (c) => `전시 전체를 압축하는 단 하나의 ${c} 시그니처 장면`,
  'system/ecosystem_proof': (c) => `${c}이(가) 하나의 시스템처럼 연결되는 과정을 보여주는 대형 미디어 월`,
  spatial_journey: (c) => `도입-핵심-마무리 동선으로 이어지는 ${c} 공간 여정 연출`,
  brand_memory: (c) => `방문 마지막에 ${c} 핵심 메시지를 남기는 참여형 기억·포토 장면`,
...
// Strategic-value subjects for direction labels: most axes use a category/value noun (NOT a facility/space/
// design word and NOT the brand name); the brand name is reserved for the leadership axis; process/operation
// axes use a process/operation word. This stops labels from being "[facility] X" or repeating the brand name.
interface DirectionSubjects {
  domainValue: string;
  productValue: string;
  brandName: string;
  processWord: string;
...
function cleanSubjectToken(value: string): string {
  let token = (value || '').replace(/[^a-zA-Z0-9가-힣]/g, '').trim();
  const stripped = token.replace(/(을|를|은|는|이|가|의|에서|에게|으로)$/, '');
  if (stripped.length >= 2) token = stripped; // drop a trailing Korean particle (밸류체인을 → 밸류체인)
  return token;
}

// Generic/abstract nouns that must never become a label subject (they carry no RFP-specific strategic value).
const GENERIC_SUBJECT_STOPWORDS = new Set(['평가자', '심사', '심사위원', '관람객', '방문객', '대상', '고객', '사용자', '참가자', '제안', '전략', '경험', '핵심', '가치', '목표', '과제', '문제', '방향', '메시지', '구성', '계획', '내용', '효과', '결과', '요구', '조건', '기준', '중심', '관련', '방안', '부분', '요소', '수단', '정보', '브랜드', '제품', '상품', '서비스', '프로젝트', '사업', '행사', '강화', '확보', '제고']);

function isFacilityOrFormatWord(token: string): boolean {
  if (!token) return true;
  return CONTEXT_NOUN_BLOCKLIST.test(token) || FACILITY_SUBSTRING.test(token) || FACILITY_LABEL_TOKENS.has(token.toLowerCase());
}

// First clean 2-8 char strategic-value noun from a candidate list (no facility/format/generic words, RFP facts, or excluded tokens).
function firstStrategicValueNoun(candidates: Array<string | undefined>, exclude: Set<string>): string {
  for (const raw of candidates) {
    for (const piece of (raw || '').split(/[\s/·,()|]+/)) {
      const token = cleanSubjectToken(piece);
      if (token.length < 2 || token.length > 8) continue;
      if (isFacilityOrFormatWord(token) || isRfpFactDirectionText(token) || GENERIC_SUBJECT_STOPWORDS.has(token) || exclude.has(token)) continue;
      return token;
    }
  }
  return '';
...
function deriveBrandName(input?: ProjectInput, bpi?: BrandProductIntelligence): string {
  return firstStrategicValueNoun([input?.clientName, bpi?.clientOrBrandRole, input?.projectName], new Set());
}
...
function buildDirectionSubjects(analysis: AnalysisResult, bpi?: BrandProductIntelligence, input?: ProjectInput, diagnosis?: RfpDiagnosis, hierarchySeeds: string[] = []): DirectionSubjects {
  ...
  const fromProduct = firstStrategicValueNoun([
...
function axisSubject(canonicalAxis: string, subjects: DirectionSubjects): string {
  switch (canonicalAxis) {
    case 'representative_position': return subjects.brandName || subjects.domainValue;
    case 'process_trust': return subjects.processWord;
    case 'operational_confidence': return '운영';
    case 'product_value_proof': return subjects.productValue || subjects.domainValue;
    case 'brand_memory':
    case 'emotional_affinity': return subjects.brandName || subjects.domainValue;
    default: return subjects.domainValue;
  }
}
...
// Map the USER-SELECTED proposalType (ProposalType, the 14-value enum) to its strategic-direction RfpConceptType
// family. This is the guardrail: the user's explicit choice — not text inference — decides which axis family is used.
// A type with no strong family (e.g. basic_proposal) maps to undefined, so evidence classification is used instead.
const PROPOSAL_TYPE_TO_RFP_CONCEPT_TYPE: Partial<Record<ProposalType, RfpConceptType>> = {
  multi_entity_pavilion: 'multi_entity_pavilion',
  corporate_technology_showcase: 'technology_showcase',
  hyundai: 'technology_showcase',
  exhibition_booth_content: 'exhibition_booth',
  visitor_center_tour: 'visitor_center_or_tour',
  popup_retail_experience: 'pop_up_or_campaign',
  mice_event_operation: 'operation_heavy_event',
  conference_forum: 'operation_heavy_event',
  brand_experience: 'single_brand_experience',
  cheil: 'single_brand_experience',
  experience_marketing: 'content_media_experience',
  innocean: 'content_media_experience',
};

// Resolve the operative RfpConceptType, preferring the user-selected proposalType as a STRONG guardrail over text
// inference (inference stays only as secondary context, never overriding the user's choice). Spec exception: never
// FORCE multi_entity_pavilion logic unless the RFP evidence actually shows multiple equal-weight entities — otherwise
// fall back to the evidence-derived type so a single-entity RFP is not pushed into pavilion logic.
function resolveGuardrailConceptType(userSelectedProposalType: ProposalType | undefined, evidenceTypes: RfpConceptType[], hasMultipleEntities: boolean): RfpConceptType {
  const evidencePrimary = primaryRfpConceptType(evidenceTypes);
  const mapped = userSelectedProposalType ? PROPOSAL_TYPE_TO_RFP_CONCEPT_TYPE[userSelectedProposalType] : undefined;
  if (!mapped) return evidencePrimary;
  if (mapped === 'multi_entity_pavilion' && !hasMultipleEntities && !evidenceTypes.includes('multi_entity_pavilion')) {
    return evidencePrimary;
  }
  return mapped;
}
...
function buildStrategicDirectionDiscoveryBrief(analysis: AnalysisResult, narrative: ProposalNarrative, conceptType: RfpConceptType, hasMultipleEntities: boolean): StrategicDirectionDiscoveryBrief {
  ...
  if (conceptType === 'multi_entity_pavilion' && hasMultipleEntities) baseAxes.push(`ecosystem_system_proof: ${compactText(narrative.differentiationPrinciple || '복수 주체의 관계와 기여를 현재 RFP 근거로 명확화', 150)}`);
  if (conceptType !== 'multi_entity_pavilion') baseAxes.push(`operational_confidence_without_multi_entity_logic: ${compactText(analysis.operationCondition || '단일 RFP 맥락의 실행 신뢰를 증명', 150)}`);

  // Context-aware winning-approach split: classify the RFP context and pick the 3 lead axes FROM that context,
  // so every RFP family (visitor-room, tech/energy, multi-entity, MICE, popup, conference, booth, ...) gets its own
  ...
  const contextAxisSets: Partial<Record<RfpConceptType, string[]>> = {
    ...
    product_experience_space: ['product_value_proof', 'category_shift', 'signature_scene'],
    technology_showcase: ['technology_reality_proof', 'representative_position', 'system/ecosystem_proof'],
    multi_entity_pavilion: ['system/ecosystem_proof', 'representative_position', 'signature_scene'],
    operation_heavy_event: ['operational_confidence', 'spatial_journey', 'evaluator_clarity'],
    pop_up_or_campaign: ['emotional_affinity', 'signature_scene', 'brand_memory'],
    exhibition_booth: ['category_shift', 'signature_scene', 'audience_understanding'],
    content_media_experience: ['audience_understanding', 'signature_scene', 'category_shift'],
    public_sector_exhibition: ['category_shift', 'audience_understanding', 'representative_position'],
  };
  const axisEvidence = (axisKey: string): string => {
    switch (axisKey) {
      case 'product_value_proof': return whatMustBeProven;
      case 'process_trust': return clientUniquePosition;
      case 'brand_memory':
      case 'emotional_affinity': return audiencePerceptionGap;
      case 'technology_reality_proof':
      case 'category_shift': return categoryShift;
      case 'representative_position': return clientUniquePosition;
      case 'system/ecosystem_proof': return whatMustBeProven;
      case 'signature_scene': return compactText(narrative.unifyingFrame || analysis.contentCondition || categoryShift, 150);
      case 'audience_understanding': return audiencePerceptionGap;
      case 'operational_confidence': return compactText(analysis.operationCondition || evaluatorDecisionRisk, 150);
      case 'spatial_journey': return compactText(analysis.spatialCondition || audiencePerceptionGap, 150);
      case 'evaluator_clarity': return evaluatorDecisionRisk;
...
function buildStrategicDirectionPlan(analysis: AnalysisResult, narrative: ProposalNarrative, hasMultipleEntities: boolean, patterns: OutlineProposalPattern[] = [], avoidanceRules: string[] = [], subjects?: DirectionSubjects, overrideConceptType?: RfpConceptType): StrategicDirectionPlanItem[] {
  const conceptTypes = classifyRfpConceptTypes(analysis, narrative, hasMultipleEntities);
  // Honor the user-selected proposalType guardrail (passed by the caller) over internal text classification.
  const conceptType = overrideConceptType ?? primaryRfpConceptType(conceptTypes);
  ...
  const directionSubjects = subjects ?? buildDirectionSubjects(analysis);
  const mk = (index: number, axis: string, rfpEvidence: string, patternLearning: string, lostAvoidance: string): StrategicDirectionPlanItem => {
    ...
    const subject = axisSubject(canonicalAxis, directionSubjects);
    return {
      type: canonicalAxis, rfpConceptType: conceptType, secondaryRfpConceptTypes: secondary, label: contextualDirectionLabel(canonicalAxis, subject),
      emphasis: directionStrategicBet(canonicalAxis, subject, rfpEvidence),
      chooseWhen: directionSelectionCriterion(canonicalAxis, subject),
...
// Strategic/structural tokens that carry no RFP CATEGORY signal — ignored when judging contamination so that a
// label built only from strategic vocabulary (e.g. "대표성 각인") is never mistaken for imported category language.
const NON_CATEGORY_LABEL_TOKENS = new Set([
  '전환', '설계', '증명', '실증', '신뢰', '경험', '가치', '방향', '대표성', '대표', '각인', '여정', '기억화', '기억',
  '공감', '정서', '압축', '핵심', '현재화', '직관', '재정의', '관점', '통합', '생태계', '운영', '체험화', '체험',
  '선언', '리더십', '몰입', '확신', '전략', '명제', '강화', '확보', '제고', '소유', '오너십', '장면', '인식', '이해',
]);

// Cross-RFP contamination guard (generic, brand-agnostic, bidirectional): a field is contamination when every
// category-bearing noun in it is absent from the current RFP evidence — i.e. its vocabulary was imported from
// another RFP category. RFP-grounded vocabulary always passes; the same word is allowed when the RFP supports it.
// Generation should never emit foreign vocabulary — this only catches and repairs model leakage in either direction.
function isCrossRfpContaminated(text: string, evidenceBlob: string): boolean {
  const evidence = evidenceBlob.toLowerCase();
  const contentTokens = (text.match(/[가-힣A-Za-z]{2,}/g) ?? [])
    .map((token) => token.toLowerCase())
    .filter((token) => !NON_CATEGORY_LABEL_TOKENS.has(token) && !GENERIC_SUBJECT_STOPWORDS.has(token));
  if (!contentTokens.length) return false;
  return contentTokens.every((token) => !evidence.includes(token));
}

// A label is "brand-generic" when it is the brand name plus a generic template form (not a leadership phrase).
const BRAND_STRONG_SUFFIX = /(리더십|대표성|대표\s*포지션|선언|주도|소유|오너십)/;
const BRAND_GENERIC_FORM = /(실체화|가치\s*증명|가치\s*체험|현재화|기억화|인식\s*전환|경험\s*이해)$|^한눈에\s*보는/;

// Card-copy validator (planner-facing): repair ONLY the weak field, never fall back to a generic template.
function validateAndRepairDirectionCards(concepts: ConceptCandidate[], plan: StrategicDirectionPlanItem[], subjects?: DirectionSubjects): ConceptCandidate[] {
  const brandName = (subjects?.brandName || '').trim();
  const domainValue = (subjects?.domainValue || '').trim();
  ...
  // RFP evidence blob (from the plan's discovery brief) used to decide whether a category term is supported.
  const evidenceBlob = plan.map((item) => [item.rfpEvidence, item.discoveryBrief?.currentProjectCategory, item.discoveryBrief?.coreRfpChallenge, item.discoveryBrief?.clientUniquePosition, item.discoveryBrief?.categoryShift, item.discoveryBrief?.whatMustBeProven, item.discoveryBrief?.audiencePerceptionGap].filter(Boolean).join(' ')).join(' ');
  return concepts.map((concept, index) => {
    ...
    // rebuild from the domain value (non-rep axes) or the leadership template (rep axis) so the brand isn't repeated.
    if (brandName.length >= 2 && label.includes(brandName)) {
      if (axis !== 'representative_position' && domainValue && domainValue !== brandName) {
        label = contextualDirectionLabel(axis, domainValue);
      } else if (axis === 'representative_position' && BRAND_GENERIC_FORM.test(label) && !BRAND_STRONG_SUFFIX.test(label)) {
        label = contextualDirectionLabel('representative_position', brandName);
      }
    }
...
function fallbackCandidate(index: number, name: string, analysis: AnalysisResult, narrative: ProposalNarrative, overrideConceptType?: RfpConceptType): ConceptCandidate {
  const conceptId = `C${index}`;
  const direction = buildStrategicDirectionPlan(analysis, narrative, Boolean(narrative.entityDifferentiationMatrix?.length && narrative.entityDifferentiationMatrix.length > 1), [], [], undefined, overrideConceptType)[(index - 1) % 3];
...
function buildFallbackConcepts(analysis: AnalysisResult, proposalNarrative: ProposalNarrative, reason: string, metadata?: ConceptGenerationMetadata, overrideConceptType?: RfpConceptType): ConceptCandidatesResult {
  const hiddenNeeds = {
  ...
  return {
    ...
    concepts: [
      fallbackCandidate(1, '', analysis, proposalNarrative, overrideConceptType),
      fallbackCandidate(2, '', analysis, proposalNarrative, overrideConceptType),
      fallbackCandidate(3, '', analysis, proposalNarrative, overrideConceptType),
    ],
    recommendation: {
      recommendedConceptId: 'C1',
      recommendedDirectionLabel: buildStrategicDirectionPlan(analysis, proposalNarrative, Boolean(proposalNarrative.entityDifferentiationMatrix?.length && proposalNarrative.entityDifferentiationMatrix.length > 1), [], [], undefined, overrideConceptType)[0]?.label || '전략 방향',
...
export async function POST(request: Request) {
  try {
    const body = (await request.json()) as {
      ...
      options?: { maxCandidates?: number; maxProposalPatterns?: number };
      conceptPromptVersion?: string;
      regenerationId?: string;
      timestamp?: string;
      attempt?: number;
    ...
    const diagnosisCompleteEnough = Boolean(
      (diag.coreWinningCondition?.trim() || diag.strategicIssue?.trim() || diag.coreProposalThesis?.trim())
      && diag.hiddenNeed?.trim()
      && diag.strategicTension?.trim()
      && (diag.persuasionTask?.trim() || diag.proofBurden?.trim())
    ...
    // Bounded strategy generation: build one compact packet from already-completed analysis outputs only.
    // This intentionally excludes full RFP/proposal/reference text, document chunks, web research, proposal patterns,
    // previous project directions, discovery regeneration, and generic fallback paths.
    const userSelectedProposalType = body.input.proposalType;
    const effectiveProposalType = userSelectedProposalType ?? body.analysis.inferredProposalType;
    const rfpConceptTypes = classifyRfpConceptTypes(body.analysis, ensureProposalNarrative(undefined, { input: body.input, analysis: body.analysis, documentText: '' }), false);
    const selectedRfpConceptType = resolveGuardrailConceptType(userSelectedProposalType, rfpConceptTypes, false);
    const selectedMatrixType = matrixTypeForRfp(selectedRfpConceptType);
    const strategyInputPacket = buildStrategyInputPacket({ input: body.input, analysis: body.analysis, rfpDiagnosis: body.rfpDiagnosis, brandProductIntelligence: body.brandProductIntelligence });

    const systemPrompt = [
      `Concept Prompt Version: ${conceptPromptVersion}.`,
      '너는 한국어 제안서 전략 방향 카드만 만드는 크리에이티브 전략가다.',
      '아래 Strategy Input Packet만 사용해 정확히 3개의 strategic direction card를 한 번에 생성한다.',
      '절대 사용 금지: full RFP document text, full uploaded proposal text, full reference proposal body, web research, proposal pattern extraction, previous project directions, proposal-type preset directions, generic fallback cards.',
      '3개 방향은 hidden need, evaluator risk, proof requirement, audience transformation, brand/product truth, signature experience opportunity 중 서로 다른 전략 선택지를 대표해야 한다.',
      '각 concepts 항목은 strategicDirectionLabel, oneLineSummary, directionAxis, whyThisDirectionExists, selectedReason, representativePersuasionScene, conceptLeap, signatureProofIdea, mainStrength, mainRisk, evidenceUsed를 반드시 포함한다.',
      'directionAxis는 allowed values 중 서로 다른 값으로만 쓴다: ' + ALLOWED_DIRECTION_AXES.join(', '),
      'strategicDirectionLabel은 2~6단어의 현재 RFP/브랜드 어휘 기반 전략명이다. 날짜/장소/클라이언트명/행사명/대상/면적/일정 같은 raw evidence fragment를 라벨로 쓰지 않는다.',
      'proposalCoreConceptName/conceptName은 호환용 임시 방향명으로 strategicDirectionLabel과 동일하게 둔다. 최종 컨셉명은 만들지 않는다.',
      '각 방향은 current RFP evidence와 brand/product evidence를 evidenceUsed 및 directionSource에 명시한다.',
      '출력은 conceptCandidatesJsonSchema에 맞추되 concepts는 정확히 3개만 반환한다. recommendation은 세 방향 중 현재 진단에 가장 맞는 하나를 중립적으로 추천한다.',
    ].join('\n');

    const userPrompt = `Strategy Input Packet (compact, authoritative, no raw documents):\n${JSON.stringify(strategyInputPacket, null, 2)}\n\nRequired output mapping:\n- concepts[0..2].strategicDirectionLabel = strategicDirectionLabel\n- concepts[0..2].oneLineStrategicBet and oneLineSummary = oneLineSummary\n- concepts[0..2].directionAxis = one distinct allowed directionAxis\n- concepts[0..2].whyThisDirectionExists = whyThisDirectionExists\n- concepts[0..2].whenToChooseThisDirection = selectedReason\n- concepts[0..2].representativePersuasionScene = representativePersuasionScene\n- concepts[0..2].conceptLeap.conceptLeap = conceptLeap\n- concepts[0..2].signatureProofIdea.whyThisProvesTheConcept and signatureScene = signatureProofIdea\n- concepts[0..2].mainStrength = mainStrength\n- concepts[0..2].mainRisk = mainRisk\n- concepts[0..2].evidenceUsed = evidenceUsed\n\nHard validation before return: reject empty direction, duplicate directionAxis, previous project contamination, proposal-type preset direction, raw evidence label, no current RFP evidence, no brand/product evidence. Do not create fallback cards.`;
    ...
    try {
      ...
      let result = normalizeConceptCandidatesResult({
        ...
        generationAttempt: metadata.generationAttempt,
        generatedAt: metadata.generatedAt,
        concepts: generated.concepts.slice(0, DEFAULT_CONCEPT_COUNT).map((concept, index) => ({
          ...concept,
          conceptId: concept.conceptId || `direction-${index + 1}`,
          rfpConceptType: concept.rfpConceptType || selectedRfpConceptType,
          strategicDirectionType: concept.strategicDirectionType || concept.directionAxis || canonicalizeDirectionAxis(concept.directionAxis, index),
          directionAxis: canonicalizeDirectionAxis(concept.directionAxis || concept.strategicDirectionType, index),
          proposalCoreConceptName: concept.proposalCoreConceptName || concept.strategicDirectionLabel,
          conceptName: concept.conceptName || concept.strategicDirectionLabel,
          conceptTitle: concept.conceptTitle || concept.strategicDirectionLabel,
          oneLineStrategicBet: concept.oneLineStrategicBet || (concept as ConceptCandidate & { oneLineSummary?: string }).oneLineSummary || concept.whatThisDirectionEmphasizes || '',
          whatThisDirectionEmphasizes: concept.whatThisDirectionEmphasizes || (concept as ConceptCandidate & { oneLineSummary?: string }).oneLineSummary || concept.oneLineStrategicBet || '',
          whenToChooseThisDirection: concept.whenToChooseThisDirection || (concept as ConceptCandidate & { selectedReason?: string }).selectedReason || '',
          representativePersuasionScene: (concept as ConceptCandidate & { representativePersuasionScene?: string }).representativePersuasionScene || concept.signatureProofIdea?.signatureScene || '',
          directionSource: {
            rfpEvidence: concept.directionSource?.rfpEvidence || compactText(JSON.stringify((concept as ConceptCandidate & { evidenceUsed?: unknown }).evidenceUsed || strategyInputPacket.coreProposalThesis), 260),
            proposalPatternLearning: concept.directionSource?.proposalPatternLearning || '사용 안 함',
            lostPatternAvoidance: concept.directionSource?.lostPatternAvoidance || '사용 안 함',
            brandProductEvidence: (concept.directionSource as typeof concept.directionSource & { brandProductEvidence?: string } | undefined)?.brandProductEvidence || strategyInputPacket.productOrServiceTruth || strategyInputPacket.brandProductSummary,
          } as ConceptCandidate['directionSource'] & { brandProductEvidence?: string },
          strategicDirectionQualityValidation: {
            isStrategicBet: true,
            isOnlyBasicRequirement: false,
            addressesCoreWinningCondition: true,
            addressesStrategicTension: true,
            addressesProofBurden: true,
            hasDistinctPointOfView: true,
            couldFitAnyRfp: false,
            validationReason: 'bounded_packet_validation_passed',
          },
        })),
      });
      ...
      result.matrixType = selectedMatrixType;
      result.primaryRfpConceptType = selectedRfpConceptType;

      const hardRejectReasons = validateBoundedDirections(result.concepts);
      if (hardRejectReasons.length) {
        console.warn('[concepts:bounded-validation-rejected]', { hardRejectReasons });
        return conceptsJson({ error: '전략 방향 생성 시간이 초과되었습니다. 분석 결과는 유지됩니다. 전략 진단을 확인하거나 다시 생성해 주세요.', reason: 'bounded_strategy_validation_failed', hardRejectReasons, conceptPromptVersion }, { status: 422 });
      }

      return conceptsJson(attachGenerationMetadata(result, metadata));
    } catch (error) {
      console.error('[concepts:bounded-generation-failed]', error);
      return conceptsJson({ error: '전략 방향 생성 시간이 초과되었습니다. 분석 결과는 유지됩니다. 전략 진단을 확인하거나 다시 생성해 주세요.', reason: 'bounded_strategy_generation_failed', conceptPromptVersion }, { status: 504 });
    }
  } catch (error) {
