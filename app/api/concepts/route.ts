...
// Cross-RFP contamination guard: a term is contamination only if it appears in the card copy but the current
// RFP evidence does NOT support it. This is RFP-grounded (NOT a Pocari-only block): the same terms are allowed
// when the RFP is actually about that category. Generation never emits these — this only catches model leakage.
const CROSS_RFP_CONTAMINATION: Array<{ terms: RegExp; evidence: RegExp }> = [
  { terms: /포카리|pocari/i, evidence: /포카리|pocari/i },
  { terms: /수분|hydration/i, evidence: /수분|음료|이온|마실|건강\s*음료|스포츠\s*음료|갈증|탈수|체액|컨디션|hydration|beverage|drink/i },
  { terms: /한\s*병|보틀|bottle/i, evidence: /병|음료|보틀|bottle|beverage|drink|마실/i },
  { terms: /몸의?\s*균형|체내\s*균형|신체\s*균형/i, evidence: /수분|음료|건강|신체|몸|전해질|운동|땀|컨디션|hydration/i },
  { terms: /전해질|이온\s*음료/i, evidence: /전해질|이온|음료|수분|hydration/i },
];
function isCrossRfpContaminated(text: string, evidenceBlob: string): boolean {
  return CROSS_RFP_CONTAMINATION.some((rule) => rule.terms.test(text) && !rule.evidence.test(evidenceBlob));
}
...
export async function POST(request: Request) {
  try {
    ...
    const systemPrompt = [
      ...
      '시설 중심 라벨 금지: 공장/견학룸/전시관/홍보관/체험관/공간/시설/룸/존/부스를 “주어”로 하거나 라벨의 끝 단어로 둔 라벨(예: 공장 실체화, 한눈에 보는 공장, 공장 가치 증명, 견학룸 인식 전환)을 만들지 말라. 라벨은 무엇을 설득하는지(제품 가치/공정 신뢰/브랜드 기억/방문객 변화 등)를 담아야 하며, 공간·시설은 설득 수단일 뿐 전략 방향의 주어가 아니다. 방문관/공장견학형이면 3개 방향이 전부 시설 소개로 수렴하지 않도록 최소 1개는 제품·브랜드 가치, 1개는 공정·품질 신뢰, 1개는 방문 후 기억·태도 변화에 배치한다.',
      '카테고리 오염 금지: 라벨/선택기준/대표장면/컨셉명은 현재 RFP가 실제로 다루는 카테고리·제품·주체 어휘에서만 가져온다. 현재 RFP가 음료/포카리/수분/공장견학 맥락이 아니면 포카리·수분·한 병·몸의 균형·전해질 같은 타 카테고리 어휘를 쓰지 말라. 수소/에너지 전시면 수소·에너지 어휘를 유지하고 방문관·공장견학 논리(제품가치/공정신뢰/방문기억 3분할)를 강제하지 말라. 다중 주체 파빌리온·MICE 운영형·팝업형도 각자의 맥락 어휘를 유지한다.',
      '클라이언트/브랜드명 반복 금지: 3개 방향 라벨이 모두 같은 브랜드/클라이언트명(예: HTWO, 현대차그룹, 포카리)으로 시작하지 않게 한다. 브랜드명은 “대표 리더십/카테고리 소유” 방향 1개에서만 주어로 쓰고(예: HTWO 리더십 각인, HTWO 대표성 강화), 나머지 방향은 설득하려는 전략 가치(수소사회 현재화, 밸류체인 압축 체험, 수분 균형의 기억, 공정 신뢰 체험 등)를 주어로 쓴다. “브랜드명 + 실체화/가치 증명/인식 전환/경험 이해/가치 체험”이나 “한눈에 보는 + 브랜드명” 형태(예: HTWO 실체화, 한눈에 보는 HTWO, HTWO 가치 증명)는 금지한다. 라벨의 주어는 RFP의 표면 명사(시설·공간·외관·디자인·리모델링·브랜드명)가 아니라 “설득되는 전략 가치”여야 한다.',
      '오염 방지(매우 중요): 음료/수분/한 병/몸의 균형/포카리/공장/제조/공정 같은 어휘는 현재 RFP가 명확히 음료·포카리·수분·공장견학·방문관·제조/제품 경험일 때만 사용한다. 그 외 RFP(수소/에너지 전시, 파빌리온, MICE, 팝업, 기술 쇼케이스, 다중 주체관 등)에서는 수분·한 병·몸의 균형·포카리 같은 표현을 라벨/선택 기준/대표 장면/컨셉명에 절대 쓰지 말고, 제품/공정/기억 3분할도 RFP가 뒷받침하지 않으면 강제하지 않는다. 항상 현재 RFP 어휘로만 작성한다.',
