...
export async function POST(request: Request) {
  try {
    ...
    const systemPrompt = [
      ...
      '반드시 다음 순서로 사고하고 출력하라: 1) Proposal Narrative 읽기 2) proposalThesis 식별 3) 핵심 audience transformation 식별 4) brand/client message 식별 5) conceptRationale 생성 6) conceptName 생성 7) conceptTagline 생성 8) conceptDefinition 생성 9) thesisProof 생성 10) experienceStructure 생성.',
      ...
      'conceptName 규칙: 짧고 기억 가능하며 발표용 제목처럼 보여야 한다. 2~5 words를 선호한다. 한국어, 영어, 한영 혼합 모두 가능하다. 섹션 제목이나 전략 설명문이 아니라 제안 콘셉트명이어야 한다. conceptName은 proposalThesis, audience transformation, client vision, HTWO/client brand message, hydrogen value chain(해당 시), strategic opportunity, core experience promise에서 의미를 끌어와야 하며, 단순 키워드 조합이나 카테고리명으로 끝나면 실패다. 긴 설명문, RFP 키워드 단순 결합, Pavilion/Zone/Experience/Journey/Hub/Platform/Showcase/Lab/Center 및 파빌리온/존/체험/여정/허브/플랫폼/쇼케이스/랩/센터/공간/전시 같은 카테고리 단어를 주 명명 장치로 쓰지 말라. "~을 위한", "~와 함께하는", "~중심의", "~기반의", "~플랫폼", "~공간", "~체험", "~전시" 구조를 쓰지 말라.',
