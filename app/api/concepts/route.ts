...
export async function POST(request: Request) {
  try {
    ...
    const systemPrompt = [
      ...
      '반드시 다음 순서로 사고하고 출력하라: 1) Proposal Narrative 읽기 2) proposalThesis 식별 3) 핵심 audience transformation 식별 4) 클라이언트/브랜드가 관람객에게 만들고 싶은 믿음 식별 5) strategic metaphor 또는 core experience promise 식별 6) conceptRationale 생성 7) conceptName 생성 8) conceptTagline 생성 9) conceptDefinition 생성 10) thesisProof 생성 11) execution strategy로서 experienceStructure 생성. 절대 booth constraints, media types, content mechanisms, object lists, RFP deliverables에서 시작하지 말라.',
      ...
      'Hydrogen/HTWO 프로젝트의 conceptRationale은 수소가 보이지 않고(invisible) 시스템 기반(system-based)이라 이해하기 어렵다는 장벽, HTWO와 Hyundai Motor Group이 future hydrogen transition의 credible leader로 인식되어야 하는 과제, invisible hydrogen system을 visible·connected·experiential하게 만드는 전시 원칙을 중심으로 작성하라. 그래서 conceptName은 synchronization, flow, transition, connection, movement toward the future 같은 전략 은유를 표현해야 하며, columns 또는 modular layout을 콘셉트의 기원으로 삼지 말라.',
      ...
      'Concept Source Priority Guard: 콘셉트는 반드시 1) client vision 2) HTWO/client brand message 3) hydrogen value chain 또는 프로젝트 고유 가치 흐름 4) audience transformation 5) strategic opportunity 6) proposal thesis 7) core experience promise에서 우선 도출하라. RFP 키워드 조합, 수소/미래/그리드/플로우 같은 범주어 조합, 외부 프로젝트명/무관한 사례명 차용으로 콘셉트를 만들지 말라.',
      ...
      'Bad conceptName 예시: Modular Interactive Hydrogen Value Chain, Beyond the Column, 기둥과 함께하는 수소 미래 체험, 공간의 제약을 기회로, Hydrogen Tech Pavilion, Future Experience Zone, Immersive Brand Experience, Hydrogen Flow, Future Nexus, Pulse of Hydrogen, Future Grid, Living H2, Hydrogen Experience, Hydrogen Pavilion, Hydrogen Journey. Better 예시 톤: Flowing Tomorrow, The Hydrogen Shift, HydroSync, Living Energy Grid, H2 in Motion, Connected Tomorrow. 단, 이 예시를 그대로 복사하지 말고 proposalThesis와 audience transformation에 맞게 새로 도출하라.',
