...
const FACILITY_LABEL_TOKENS = new Set(['공장', 'factory', 'plant', '시설', 'facility', '견학룸', '견학', '전시관', '홍보관', '체험관', '쇼룸', 'showroom', '매장', '점포', 'store', '공간', 'space', '룸', 'room', '존', 'zone', '관', 'hall', '부스', 'booth', '파빌리온', 'pavilion']);
...
function isGenericDirectionLabel(label: string): boolean {
  ...
  // Facility-anchored anywhere (subject, object, or embedded token) → facility-centered, not a strategic direction.
  return tokens.some((token) => FACILITY_LABEL_TOKENS.has(token.toLowerCase()));
}
...
const FACILITY_SUBSTRING = /(공장|시설|견학|쇼룸|매장|점포|factory|plant|facility|showroom|파빌리온|pavilion|부스|booth)/i;

// A short, clean subject noun from the current RFP to seed user-facing labels (no facts/figures/content-formats/facilities).
function directionContextNoun(analysis: AnalysisResult): string {
  const seed = fallbackNameSeeds(analysis).find((token) => token.length >= 2 && token.length <= 8 && !isRfpFactDirectionText(token) && !CONTEXT_NOUN_BLOCKLIST.test(token) && !FACILITY_SUBSTRING.test(token));
  return seed || '브랜드';
}
...
// The label/copy subject by axis role: process/operation axes use a process/operation word (공정/운영),
// so a process-trust direction reads as "공정 신뢰 체험화" rather than borrowing the facility/product noun.
function axisSubject(canonicalAxis: string, contextNoun: string): string {
  if (canonicalAxis === 'process_trust') return '공정';
  if (canonicalAxis === 'operational_confidence') return '운영';
  return (contextNoun || '브랜드').trim() || '브랜드';
}
...
function buildStrategicDirectionDiscoveryBrief(analysis: AnalysisResult, narrative: ProposalNarrative, conceptType: RfpConceptType, hasMultipleEntities: boolean): StrategicDirectionDiscoveryBrief {
  ...
  // Context-aware winning-approach split. Apply the factory-tour/visitor-room split (product value / process
  // trust / brand memory) ONLY for clear tour/room RFPs. Tech/energy stays around technology reality / leadership /
  // value-chain. Everything else keeps the default discovery order (no overfit to one RFP family).
  ...
  const leadAxes = isVisitorRoomFactory
    ? [`product_value_proof: ${whatMustBeProven}`, `process_trust: ${clientUniquePosition}`, `brand_memory: ${audiencePerceptionGap}`]
    : isTechShowcase
      ? [`technology_reality_proof: ${categoryShift}`, `representative_position: ${clientUniquePosition}`, `system/ecosystem_proof: ${whatMustBeProven}`]
      : [];
...
function buildStrategicDirectionPlan(analysis: AnalysisResult, narrative: ProposalNarrative, hasMultipleEntities: boolean, patterns: OutlineProposalPattern[] = [], avoidanceRules: string[] = []): StrategicDirectionPlanItem[] {
  ...
  const mk = (index: number, axis: string, rfpEvidence: string, patternLearning: string, lostAvoidance: string): StrategicDirectionPlanItem => {
    ...
    const subject = axisSubject(canonicalAxis, contextNoun);
    return {
