...
function buildStrategicDirectionPlan(analysis: AnalysisResult, narrative: ProposalNarrative, hasMultipleEntities: boolean, patterns: OutlineProposalPattern[] = [], avoidanceRules: string[] = []): StrategicDirectionPlanItem[] {
  ...
  const mk = (index: number, axis: string, rfpEvidence: string, patternLearning: string, lostAvoidance: string): StrategicDirectionPlanItem => ({
    ...
    emphasis: `${axis} 관점에서 현재 RFP의 winning condition을 공간·콘텐츠·미디어·운영 proof로 증명합니다. 근거: ${rfpEvidence}`,
    chooseWhen: `이 RFP의 선택 이유가 ${rfpEvidence}를 평가자가 빠르게 이해하고 믿는 데 달려 있을 때 선택합니다.`,
...
export async function POST(request: Request) {
  try {
    ...
    const systemPrompt = [
      ...
      'oneLineStrategicBet은 반드시 “이 방향은 ___을 통해 ___을 설득하는 전략입니다.” 형식의 1문장으로 쓴다. whenToChooseThisDirection은 반드시 “클라이언트가 ___을 가장 중요하게 볼 때 선택합니다.”에 가까운 실무 선택 기준으로 쓴다. signatureProofIdea의 대표 장면/콘텐츠는 카드에서 짧게 읽히도록 구체 구 하나로 요약 가능해야 한다.',
      ...
      '3개 전략 방향은 반드시 (1) 증명하려는 것 (2) 해결하는 평가자 우려 (3) signature proof 종류 (4) 사용자가 선택할 상황이 달라야 한다. 세 방향이 같은 전략의 표현 차이, 일반 요구 충족, 추상적 설명이면 반환 전에 재생성한다.',
