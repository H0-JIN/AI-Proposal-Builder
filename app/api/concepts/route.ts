...
export async function POST(request: Request) {
  try {
    ...
    const hasMultipleEntities = Boolean(proposalNarrative.entityDifferentiationMatrix?.length && proposalNarrative.entityDifferentiationMatrix.length > 1);
    ...
    const systemPrompt = [
      ...
      'If the RFP contains multiple entities, do not solve the proposal only with unity. Define what is unified and what remains distinct, then make the distinction visible in conceptRationale, thesisProof, and experienceStructure.',
      ...
      'Anti-pattern validation rule: lost proposal avoidance guidance such as “entities were not differentiated clearly” means 검증 질문(예: 각 entity의 역할과 proof가 명확한가?)으로만 전환하라. Distinct Unity, Synergized Distinction, Focused Identity처럼 avoidance rule을 직접 반복한 conceptName을 만들면 실패다.',
