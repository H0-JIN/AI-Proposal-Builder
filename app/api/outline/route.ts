...
export async function POST(request: Request) {
  try {
    const body = (await request.json()) as { input: ProjectInput; analysis: AnalysisResult; selectedConcept: ConceptCandidate; conceptDevelopmentLogic?: ConceptDevelopmentLogic; conceptCandidates?: ConceptCandidate[]; conceptRecommendation?: ConceptRecommendation };
    ...
    const productCodes = extractProductCodes({ input: body.input, analysis: body.analysis, selectedConcept: body.selectedConcept, conceptDevelopmentLogic: body.conceptDevelopmentLogic, conceptCandidates: body.conceptCandidates, conceptRecommendation: body.conceptRecommendation });
    ...
    const result = await createStructuredJson<{ slides: SlideOutline[] }>({
      ...
      system: [
        ...
        '사용자가 이미 선택한 콘셉트를 기준으로 구조를 설계하라. 반드시 Concept Development Logic, Concept Candidates, Selected Concept, Core Concept, Key Experience Asset Concept 장표 흐름을 포함하라. 후보 비교 장표는 새 후보를 만들지 말고 전달받은 conceptCandidates 3안을 비교하는 용도로만 사용하라. 고정 제목 “Monument Design Concept”은 사용하지 말라.',
        ...
        'Spatial / Content Plan은 절대 1장으로 요약하지 말고 최소 5장(Spatial Overview, Main Experience Image, 실제 체험명 기반 상세 장표 2장 이상, Experience Scenario)으로 구성하라. Zone Detail 01 같은 일반 제목 대신 Q8 멀티태스킹 챌린지, H8 4:3 몰입 콘텐츠 체험, B8 플렉스캠 셀피 스튜디오처럼 실제 체험명을 slideTitle로 사용하라.',
        ...
        'RFP나 분석 결과의 taskSections.requiredDeliverables/requiredScope/productInfo에 제품/서비스 단위가 있으면 그 단위별 Product Experience Detail 장표를 반드시 포함하라. referenceOnly/doNotTreatAsScope/existingAssets에서만 감지된 참고 사례, 기존 캠페인, 레슨런드 항목은 제품별 체험 상세 장표로 만들지 말라. 포괄적인 “폴더블 갤럭시 체험존” 대신 대화면 멀티태스킹 체험, 미디어 최적화 폼팩터 체험, 전면 디스플레이 셀피 체험처럼 방문객 행동 중심의 구체 제목을 사용하라.',
...
콘셉트 후보 3안 JSON:
${JSON.stringify(body.conceptCandidates ?? [], null, 2)}

AI 추천 JSON:
${JSON.stringify(body.conceptRecommendation ?? null, null, 2)}
...
- 감지된 제품/콘텐츠 코드: ${productCodes.length ? productCodes.join(' / ') : '없음'}`,
    ...
    return NextResponse.json(expandExperiencePlanOutline(result.slides, { input: body.input, analysis: body.analysis, selectedConcept: body.selectedConcept, conceptDevelopmentLogic: body.conceptDevelopmentLogic, conceptCandidates: body.conceptCandidates, conceptRecommendation: body.conceptRecommendation }));
  } catch (error) {
