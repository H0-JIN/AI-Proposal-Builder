  ...
  try {
    ...
    const result = await createStructuredJson<{ slides: SlideOutline[] }>({
      ...
      system: [
        ...
        '방문관/공장견학/쇼룸/브랜드 체험형에서는 requiredDeliverables/requiredScope의 공간·시공 항목(견학룸·로비·사무실·외부·입구·인테리어·리모델링·공간 개선·외관 개선·미디어 설치 등)을 그대로 콘텐츠 장표 제목/주제로 만들지 말라(예: "사무실 리모델링", "로비 개선", "외부 입구 개선" 같은 시공 항목 제목 금지). 각 항목을 방문객 경험 모듈로 재구성하라: 방문 동선 역할(입장 의례 · 오리엔테이션 · 제품 진실 발견 · 증명 · 감각 경험 · 참여/개인화 · 방문 후 기억) + 제품 진실 + 증명 장치 + 브랜드 감각 세계 + 공정 가시화 + 미디어/인터랙션 모듈 관점으로 전환한다. 콘텐츠 장표는 "무엇을 리모델링하나"가 아니라 "방문객이 무엇을 경험·이해·확신하나, 이 콘텐츠가 어떤 증명을 주나, 최종 컨셉을 어떻게 표현하나, 어떤 이미지/미디어/인터랙션으로 기억되나, 어떻게 수주에 기여하나"를 다룬다. 각 콘텐츠 장표는 contentModuleName(경험 모듈명) · visitorAction(방문객 행동) · proofValue(증명 가치) · mediaOrExperienceType(미디어/경험 방식) · relationToFinalConcept(최종 컨셉 연결) · relationToRfpRequirement(원 RFP 요구 연결) · heroElement · visualDirection · feasibilityNote 관점으로 작성하고, 기존 SlideContent 필드(visitorAction/visitorMission/systemResponse/spatialPlacement/outputOrReward/experienceTitle)에 매핑한다.',
        '리모델링·공간 개선·외관·인테리어·내부 개선·입구 개선·시공 같은 시공/건축 요구는 본문 콘텐츠 내러티브를 지배하지 말라. 이런 요구는 실행/타당성/공간 구현 디테일(Execution / Operation / Feasibility 또는 Space Execution 섹션, 또는 상세/부록 페이지)로 보내 일정·범위·구현 방식·feasibilityNote로 정리하고, 메인 콘텐츠 장표는 방문객 경험·제품 진실·증명 모듈로 채운다. 단, 다중 주체/공동관형·국가관/엑스포형에는 이 방문객 모듈 재구성을 강제하지 말고 기존 유형별 구조를 유지한다.',
