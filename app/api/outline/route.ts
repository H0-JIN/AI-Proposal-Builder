...
export async function POST(request: Request) {
  try {
    ...
    const outlineEvidenceGroups = retrieveCategoryEvidenceGroups({
      ...
      groups: [
        { label: '필수 목차', categories: ['requiredDeliverables'], description: 'requiredDeliverables를 필수 목차/요구 대응 장표로 매핑', limit: 4 },
        { label: 'KPI 대응', categories: ['kpi'], description: 'KPI/성과 측정 장표와 대응 메시지 생성에 사용', limit: 3 },
        { label: '평가 대응 전략', categories: ['evaluationCriteria'], description: '평가 기준별 대응 전략 및 차별화 장표에 사용', limit: 3 },
        { label: '실행 전략', categories: ['constraints', 'existingAsset'], description: '제약 조건과 기존 자산을 실행 가능성/운영 계획에 반영', limit: 4 },
        { label: '공간 전략', categories: ['venue'], description: '공간 구성, 동선, 장소별 적용 장표에 사용', limit: 3 },
      ],
    });
    ...
    const result = await createStructuredJson<{ slides: SlideOutline[] }>({
      ...
      system: [
        ...
        '아웃라인 retrieval은 category별 목적에 맞춰 사용하라. requiredDeliverables는 필수 목차와 RFP Requirement Response, kpi는 KPI 대응 및 성과 측정, evaluationCriteria는 평가 대응 전략, constraints/existingAsset은 실행 전략과 운영/제작 가능성, venue는 공간 전략과 동선/장소 적용 장표에만 우선 사용한다.',
        '필수 category 근거가 있는 경우 해당 장표를 반드시 구조에 반영하되, referenceOnly나 backgroundInsight에서만 나온 항목을 requiredDeliverables, KPI, 평가 기준, 필수 공간 장표로 승격하지 말라.',
