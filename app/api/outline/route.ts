  ...
  try {
    ...
    const result = await createStructuredJson<{ slides: SlideOutline[] }>({
      ...
      system: [
        ...
        'analysis.requiredDeliverables의 각 항목은 최소 1개 이상의 slide 또는 slide section에 반드시 연결하라. 중요도 판단으로 임의 생략하지 말고, 하나의 장표에 묶더라도 항목 자체를 slidePurpose/keyMessage/mainCopy에 명시하라.',
        ...
        'requiredDeliverables 또는 scopeOfWork에 누락이 있으면 아웃라인에 대응 장표를 자동 보완하라. 예: 템플릿에 없더라도 필수 항목에 KV 및 PPT 템플릿 제안이 있으면 Design / KV Direction 장표를 생성하라.',
        ...
        'Spatial / Content Plan은 절대 1장으로 요약하지 말고 최소 5장(Spatial Overview, Main Experience Image, 실제 체험명 기반 상세 장표, Experience Scenario)으로 구성하라. 동일 제품/동일 체험 상세 장표는 중복 생성하지 말고 제품당 1~2장으로 제한하라. Q8/H8/B8처럼 복수 제품이 있으면 제품별 상세 장표가 균형 있게 나오도록 배치하고, 포괄적인 폴더블 갤럭시 제품 체험 모듈 하나로 병합하지 말라. 같은 제품의 상세 장표가 여러 개 필요할 때는 Q8 체험 개요, Q8 체험 시나리오처럼 역할을 명확히 분리하라.',
        'Media / Interactive Plan은 절대 1장으로 요약하지 말고 최소 5장(Media Experience Overview, Key Media Scene, Interactive Flow, Content Mechanism, Output & Share)으로 구성하라. Content Mechanism과 콘텐츠 작동 원리 및 메커니즘처럼 같은 의미의 장표를 중복 생성하지 말라. 미디어/인터랙션 요소가 많으면 핵심 체험 자산별 상세 장표를 추가하라.',
...
유형별 구조 가이드: ${styleGuides[body.analysis.inferredProposalType ?? body.input.proposalType]}
