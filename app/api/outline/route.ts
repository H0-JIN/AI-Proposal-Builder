  ...
  try {
    ...
    const result = await createStructuredJson<{ slides: SlideOutline[] }>({
      ...
      system: [
        ...
        'Core Concept 이전에는 반드시 1) Project / Market Context 2) Core Problem or Challenge 3) Audience Insight 4) Case Insight / Benchmark Insight(콘셉트 근거를 강화할 때만) 5) Strategic Opportunity / Strategic Direction 6) Concept Rationale 순서가 먼저 설명되어야 한다. Core Concept 장표는 왜 이 콘셉트가 필요한지 설명한 뒤에만 배치하고, Audience Insight 또는 Strategic Direction보다 앞에 두지 말라.',
        'Concept Rationale은 공간 제약에서 시작하지 말고 1) 관람객이 이해하기 어려운 것 2) 클라이언트가 관람객에게 믿게 해야 하는 것 3) 프로젝트의 전략적 기회 4) 간극을 해결하는 경험 원칙 5) 선택 콘셉트가 그 원칙을 가장 잘 표현하는 이유 순서로 작성하라. Case Insight가 유용할 때는 Concept Rationale 바로 앞의 전략 근거로만 배치하고 콘텐츠 제안 섹션 뒤에 두지 말라. Hydrogen/HTWO 프로젝트는 수소가 보이지 않고 시스템 기반이라 어렵다는 점, 생산·저장·운송·사용으로 이어지는 value chain, Hyundai Motor Group의 통합 미래 에너지 시스템, HTWO의 hydrogen transition 리더십 신뢰, 연결된 시스템을 보이는 경험화에 집중하라.',
