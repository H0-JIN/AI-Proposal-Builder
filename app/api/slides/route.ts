  ...
  try {
    ...
    const result = await createStructuredJson<{ slides: SlideContent[] }>({
      ...
      system: [
        ...
        isEventOperationType ? '이 단계는 행사 운영형 제안 생성 단계다. 사용자가 수정한 슬라이드 아웃라인을 최종 기준으로 삼아 RFP 요약을 반복하지 말고 행사 목적, 브랜드 메시지, 프로그램, 등록/입장, 세션 시스템, 파트너 부스, 네트워킹/케이터링, 동선, 설치/전환, 인력, 리스크, 예산 문안을 실제 제안서 초안 수준으로 생성하라.' : structureGuard.proposalScopeTypes.includes('contentDevelopment') ? '이 단계는 콘텐츠 개발형 제안 생성 단계다. 사용자가 수정한 슬라이드 아웃라인을 최종 기준으로 삼아 RFP 요약을 반복하지 말고 content concept, narrative, media mechanism, hero content, sub content, scenario, schedule, credential 중심의 PPT 장표 문안을 실제 제안서 초안 수준으로 생성하라.' : '이 단계는 제안 생성 단계다. 사용자가 수정한 슬라이드 아웃라인을 최종 기준으로 삼아 RFP 요약을 반복하지 말고 경험 전략, 콘셉트, 핵심 체험 자산, 공간/콘텐츠 구성, 미디어/인터랙션, 방문객 여정, PPT 장표 문안을 실제 제안서 초안 수준으로 생성하라. 아웃라인의 slideTitle, slidePurpose, keyMessage, mainCopy 수정 내용은 반드시 반영하라.',
        '각 슬라이드는 Proposal Narrative와 outline metadata를 보존한다. slidePurpose는 Problem, Insight, Strategy, Concept, Experience, Content, Proof, Impact 중 하나만 사용하고 slideRole, relationToThesis, whyThisSlideExists를 반드시 작성하라. Case Insight 장표는 Concept Rationale 이전의 전략 근거로만 작성하고 콘텐츠 제안 섹션 이후의 reference/supporting page처럼 쓰지 말라.',
        `각 슬라이드는 slideNumber, slideType, slideTitle, slidePurpose, slideRole, relationToThesis, whyThisSlideExists, sourceEvidence, referenceAllowed, keyMessage, mainCopy, bodyBullets, visualDirection, visitorAction, contentMechanism, spatialPlacement, mediaOrObject, outputOrReward, imagePlaceholder, visualPrompt, diagramSuggestion, productExperienceDetails, keyExperienceAssets, experienceScenarioSteps, referenceInsights, speakerNote, confirmNeededNote를 모두 작성한다. 일반 슬라이드에서 해당 배열이 없으면 빈 배열을 넣는다. sourceEvidence는 문자열 배열이며 현재 프로젝트 근거가 없으면 []로 채우고, referenceAllowed는 Reference Guard가 허용한 현재 프로젝트 레퍼런스 근거가 있을 때만 true다. 제품/콘텐츠/entity 상세 장표는 ${experienceDetailFields.join(', ')} 항목을 productExperienceDetails에 명확히 작성하라. Entity Differentiation Matrix가 있으면 각 detail slide의 productRole, coreValue, visitorMission, visitorAction, systemResponse, contentMechanism은 해당 entity의 roleInProject, keyOffering, audienceTakeaway, spatialOrContentRole, experienceMechanism에서 파생하라.`,
        ...
        'Case Insight 문안은 reference list가 아니라 insightSummary, implicationForCurrentProject, connectionToConcept, relationToThesis, whyThisSlideExists를 명시적으로 답해야 한다. current evidence로 검증되지 않은 case/project/brand/exhibition name은 쓰지 말고, 필요하면 “복잡한 기술 전시”, “브랜드 경험 공간”, “혼합 B2B/Public 관람 상황”처럼 추상 사례 원칙으로 표현하라.',
        ...
        '제안 아이디어와 장표 문안은 analysis.requiredDeliverables, analysis.scopeOfWork, analysis.taskSections[].requiredDeliverables를 최우선 기준으로 삼고 analysis.requiredScope, analysis.productInfo, analysis.productFeatures 중심으로만 생성하라. proposalType별 템플릿보다 RFP 필수 항목과 과업 범위가 우선이다. analysis.referenceOnly, analysis.doNotTreatAsScope, analysis.existingAssets 항목은 독립 체험 모듈/제품 상세/신규 콘텐츠 단위 또는 기본 참고 장표로 생성하지 말고 배경 맥락으로만 사용하라.',
        ...
        'Evaluation Criteria 관련 장표와 각 chapter 문안은 analysis.evaluationCriteria 및 evaluationCriteria retrieval chunk를 근거로 심사 기준에 어떻게 대응하는지 드러나게 작성하라. Reference Insight, Design Reference Direction, 참고 방향 및 레퍼런스 인사이트 장표는 기본 생성하지 말라. 사용자가 레퍼런스 장표를 명시적으로 요청했거나 제안 콘텐츠/미디어 메커니즘/공간 경험의 추가 설명·검증에 반드시 필요한 경우에만 현재 프로젝트 evidence에 명시된 reference를 referenceInsights 배열에 작성하고 referenceName, referenceType, whatToLearn, howToApply, caution, sourceEvidence, referenceAllowed를 채워라. sourceEvidence가 없거나 referenceAllowed=false이면 해당 reference와 장표를 제거하라. Case Insight는 referenceInsights 목록을 채우는 장표가 아니며, 유사 사례의 원칙을 현재 콘셉트 전략으로 번역하는 본문 장표로 작성하라. caution에는 “실제 제안 범위가 아닌 참고 사례”라는 의미가 분명히 드러나야 한다.',
        '참고 사례를 다룰 때도 현재 프로젝트 evidence에 명시된 이름만 사용하고, 해당 reference가 설명하는 제안 콘텐츠, 참고하는 구체 메커니즘/경험 원칙, 제안 강화 방식을 함께 답하라. 근거 없는 프로젝트명이나 기존 캠페인명을 신규 모듈 장표 또는 productExperienceDetails로 만들지 말라.',
        ...
        '제품 또는 주요 콘텐츠/entity 단위는 Entity Differentiation Matrix와 analysis.productFeatures의 product/keyFeature/valueProposition을 우선 반영하고 analysis.requiredDeliverables, analysis.scopeOfWork, analysis.taskSections.requiredDeliverables, analysis.requiredScope, analysis.productInfo 또는 analysis.productFeatures에 명시된 제품/서비스 단위만 기준으로 삼아 각 단위별 Product Experience Detail 장표를 반드시 생성하라. Q8/H8/B8처럼 제품 코드가 복수로 감지되면 Q8, H8, B8 각각의 상세 장표 또는 제품별 비교표를 만들고, “폴더블 갤럭시 제품 체험 모듈”처럼 포괄 이름 하나로 병합하지 말라. 동일 제품/동일 체험 장표는 중복 생성하지 말고 제품당 1~2장으로 제한하라. Q8/H8/B8처럼 복수 제품이 있으면 한 제품에 상세 장표가 몰리지 않도록 균형 있게 배치하라. 같은 제품에 2장이 필요할 때만 “체험 개요”와 “체험 시나리오”처럼 역할을 명확히 분리하라. productExperienceDetails 배열에는 productCode, productRole, coreValue, experienceTitle, oneLineExperience, visitorMission, visitorAction, systemResponse, mediaOrObject, spatialPlacement, outputOrReward, snsSharePoint, visualDirection, imagePlaceholder, diagramSuggestion을 채워라. 단순 제품 설명이 아니라 방문객 행동, 시스템 반응, 결과물이 명확한 콘텐츠만 작성하라. 모든 entity에 visitor mission → kiosk exploration → video explanation → result report 같은 재사용 템플릿을 반복하지 말고, 각 entity의 역할/제공 가치/증거에서 고유한 content mechanism을 도출하라. referenceOnly/doNotTreatAsScope/existingAssets의 참고 사례, 기존 캠페인, 레슨런드 항목은 제외하라. “제작”, “개발”, “운영”, “구성”, “기획”, “제안” 같은 과업/업무 범위 표현은 체험 콘텐츠명으로 사용하지 말고 실행 계획, 제작 범위, 운영 계획 장표에서만 다루라.',
        ...
        'Media / Interactive Plan은 각 entity별 차이가 보이도록 관람객 행동 → 센서/입력/운영 접점 → 미디어/시스템 반응 → 결과물 → 공유가 보이도록 visitorAction, contentMechanism, mediaOrObject, outputOrReward, diagramSuggestion을 연결해 작성하라.',
...
RFP Entity Differentiation Matrix / Strategy:
