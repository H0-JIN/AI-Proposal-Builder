...
import type { AnalysisResult, ConceptCandidate, ConceptDevelopmentLogic, ConceptRecommendation, ProjectInput, SlideContent, SlideOutline } from '@/lib/types';
...
export async function POST(request: Request) {
  try {
    const body = (await request.json()) as { input: ProjectInput; analysis: AnalysisResult; selectedConcept: ConceptCandidate; outline: SlideOutline[]; conceptDevelopmentLogic?: ConceptDevelopmentLogic; conceptCandidates?: ConceptCandidate[]; conceptRecommendation?: ConceptRecommendation };
    ...
    const expandedOutline = expandExperiencePlanOutline(body.outline, { input: body.input, analysis: body.analysis, selectedConcept: body.selectedConcept, conceptDevelopmentLogic: body.conceptDevelopmentLogic, conceptCandidates: body.conceptCandidates, conceptRecommendation: body.conceptRecommendation });
    ...
    const result = await createStructuredJson<{ slides: SlideContent[] }>({
      ...
      system: [
        ...
        '사용자가 선택한 콘셉트만 이후 실행 장표의 기준으로 작성하라. 단, Concept Development Logic 장표에는 conceptDevelopmentLogic의 핵심 과제, 타깃 인사이트, 브랜드/제품 가치, 공간/경험 기회, 컨셉 개발 기준을 반영하고, Concept Candidates 장표에는 전달받은 3개 후보를 콘셉트명, 핵심 메시지, 경험 구조, 강점, 리스크, 추천 여부 비교표처럼 정리하라. Selected Concept/Core Concept 장표에는 selectedConcept의 conceptNameKR, conceptNameEN, oneLineDefinition, coreMessage, experienceLogic, targetRelevance, whyThisWorks, riskOrCaution이 명확히 드러나게 작성하라.',
...
- 감지된 제품/콘텐츠 코드: ${productCodes.length ? productCodes.join(' / ') : '없음'}`,
    ...
    return NextResponse.json(sanitizeKpiSlides(result.slides, body.analysis));
  } catch (error) {
