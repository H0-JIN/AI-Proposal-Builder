  ...
  try {
    ...
    const result = await createStructuredJson<{ slides: SlideContent[] }>({
      ...
      system: [
        ...
        '제품 또는 주요 콘텐츠 단위가 analysis.taskSections.requiredDeliverables, analysis.requiredScope 또는 analysis.productInfo에 명시되어 있으면 각 단위별 Product Experience Detail 장표를 반드시 생성하고 productExperienceDetails 배열에 productCode, productNameOrRole, coreValue, experienceTitle, oneLineExperience, visitorMission, visitorAction, contentMechanism, mediaOrObject, spatialPlacement, outputOrReward, snsSharePoint, visualDirection, imagePlaceholder, diagramSuggestion을 채워라. referenceOnly/doNotTreatAsScope 항목은 제외하라.',
