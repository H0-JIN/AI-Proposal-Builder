  ...
  try {
    ...
    const result = await createStructuredJson<{ slides: SlideContent[] }>({
      ...
      system: [
        ...
        `각 슬라이드는 slideNumber, slideType, slideTitle, slidePurpose, slideRole, relationToThesis, whyThisSlideExists, sourceEvidence, referenceAllowed, keyMessage, mainCopy, bodyBullets, visualDirection, visitorAction, contentMechanism, spatialPlacement, mediaOrObject, outputOrReward, imagePlaceholder, visualPrompt, diagramSuggestion, productExperienceDetails, keyExperienceAssets, experienceScenarioSteps, referenceInsights, speakerNote, confirmNeededNote를 모두 작성한다. 일반 슬라이드에서 해당 배열이 없으면 빈 배열을 넣는다. sourceEvidence는 문자열 배열이며 현재 프로젝트 근거가 없으면 []로 채우고, referenceAllowed는 Reference Guard가 허용한 현재 프로젝트 레퍼런스 근거가 있을 때만 true다. 제품/콘텐츠 상세 장표는 ${experienceDetailFields.join(', ')} 항목을 productExperienceDetails에 명확히 작성하라.`,
