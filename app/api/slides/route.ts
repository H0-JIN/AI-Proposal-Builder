  ...
  try {
    ...
    const result = await createStructuredJson<{ slides: SlideContent[] }>({
      ...
      system: [
        ...
        `Key Experience Asset Concept 슬라이드에는 selectedConcept.keyExperienceAssetDirection을 기준으로 프로젝트 핵심 체험 자산을 ${assetTypeGuide} 중 하나 또는 복수로 자동 판단하고 assetType, assetName, roleInExperience, designDirection, visitorAction, contentMechanism, mediaOrMaterial, spatialPlacementIdea, interactionPoint, visualPrompt, expectedVisitorMemory 항목이 드러나도록 작성하라.`,
        ...
        'Spatial / Content Plan은 선택된 콘셉트와 핵심 체험 자산을 기준으로 최소 5장 구조를 반드시 유지한다: Spatial Overview, Main Experience Image, Zone Detail 01, Zone Detail 02, Experience Scenario. 핵심 체험 자산이 3개 이상이면 Zone Detail 03 이상으로 추가된 아웃라인을 따라 상세화하라.',
