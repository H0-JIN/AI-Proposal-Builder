...
export async function POST(request: Request) {
  try {
    const body = (await request.json()) as { input: ProjectInput; analysis: AnalysisResult; outline: SlideOutline[]; selectedConcept: ConceptCandidate };

    if (!body.input || !body.analysis || !body.outline?.length || !body.selectedConcept) {
      return NextResponse.json({ error: '프로젝트 입력값, 분석 결과, 아웃라인, 선택된 콘셉트가 필요합니다.' }, { status: 400 });
    }
    ...
    const result = await createStructuredJson<{ slides: SlideContent[] }>({
      ...
      system: [
        ...
        '본문 문안에는 RFP Fact / AI Proposal / Confirm Needed 구분을 반영하라. 단, AI Proposal 영역은 RFP 반복이 아니라 선택된 콘셉트를 기준으로 한 새 제안 아이디어여야 하며 Confirm Needed는 confirmNeededNote에만 배치하라.',
        'Concept Candidates 비교 장표를 새로 만들지 말고, 선택된 콘셉트의 conceptNameKR, conceptNameEN, oneLineDefinition, coreMessage, experienceLogic, keyExperienceAssetDirection을 Core Concept 장표에 반영하라.',
        `Key Experience Asset Concept 슬라이드에는 선택된 콘셉트의 keyExperienceAssetDirection을 기준으로 프로젝트 핵심 체험 자산을 ${assetTypeGuide} 중 하나 또는 복수로 자동 판단하고 assetType, assetName, roleInExperience, designDirection, visitorAction, contentMechanism, mediaOrMaterial, spatialPlacementIdea, interactionPoint, visualPrompt, expectedVisitorMemory 항목이 드러나도록 작성하라.`,
