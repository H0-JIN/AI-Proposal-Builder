...
export type DbSupportedExtension = (typeof dbSupportedExtensions)[number];
...
export type PptxSlideExtraction = {
  slideNumber: number;
  title?: string;
  text: string;
};
...
export async function extractPptxSlides(buffer: Buffer): Promise<PptxSlideExtraction[]> {
...
export type TextExtractionMode = 'rfp' | 'db';

export type ExtractedPageQuality = PdfPageExtraction & {
  useVision: boolean;
  reasons: string[];
};
...
export async function extractDocumentTextFromBuffer({
...
}): Promise<{ result: ExtractDocumentTextResult; httpStatus: number }> {
  ...
  if (extension === 'docx') {
    ...
    if (!validation.ok) {
      ...
      return { result: { [key]: validation.message }, httpStatus: 422 };
    }

    return { result: { text: validation.text, status: 'success' }, httpStatus: 200 };
  }
  ...
  if (extension === 'pptx') {
    ...
    if (!validation.ok) {
      const key = validation.reason === 'short' ? 'warning' : 'error';
      return { result: { [key]: validation.message, slides }, httpStatus: 422 };
    }
  ...
  const pageQuality = assessExtractedPdfPages(pdfExtraction.pages, fileSize);
  const qualityAssessment = assessExtractedTextQuality(pdfExtraction.text, fileSize);
