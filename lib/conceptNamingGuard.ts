...
function hasStrategyStatementName(name: string) {
  ...
  return [
    ...
    /실체화/u,
    /한눈에\s*보는/u,
    /(core\s*experience|insight\s*hub|panorama|signature|moment)\s*$/i,
    /구현$/u,
    /전략$/u,
    /방향$/u,
    /솔루션$/u,
...
export function buildConceptNamingRetryInstruction(violations: string[]) {
  return [
    ...
    'Also reject these generic strategy-statement / consulting-branding forms as the core concept name (unless transformed into a specific RFP-grounded idea): 가치 증명, 기억의 증명, 인식 전환, 경험 이해, 가치 체험, 실체화, 한눈에 보는 ___, ___ 중심, ___ 시그니처, ___ Core Experience, ___ Insight, ___ Panorama, ___ Signature, ___ Experience, ___ Journey, ___ Moment, and a brand/client name plus a single generic noun.',
